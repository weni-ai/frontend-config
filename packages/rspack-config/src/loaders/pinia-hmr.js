 ...
 *
 * Kept as CommonJS (`src/loaders/package.json` sets `"type": "commonjs"`)
 * because Rspack resolves loaders via `require()`.
 */
module.exports = function piniaHmrLoader(source) {
  ...
  let code = source
    ...
    .replace(
      /import\s*\{\s*registerStoreHMR\s*\}\s*from\s*['"]@weni\/rspack-config\/hmr['"];?\r?\n?/g,
      '',
    )
    .replace(
  ...
  const importStatement = `import { registerStoreHMR } from '@weni/rspack-config/hmr';\n`;
